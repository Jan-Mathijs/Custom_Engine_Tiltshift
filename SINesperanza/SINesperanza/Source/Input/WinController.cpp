...
namespace SIN
{
	...
	void WinController::UpdateStatus()
	{
		...
		{
		case winDpadUp:
			m_controllerState.DpadState |= DpadFlags::DpadUp;
			break;
		case winDpadRight:
			m_controllerState.DpadState |= DpadFlags::DpadRight;
			break;
		case winDpadDown:
			m_controllerState.DpadState |= DpadFlags::DpadDown;
			break;
		case winDpadLeft:
			m_controllerState.DpadState |= DpadFlags::DpadLeft;
			break;
		case winDpadUpRight:
			m_controllerState.DpadState |= DpadFlags::DpadUp;
			m_controllerState.DpadState |= DpadFlags::DpadRight;
		...
		{
			m_controllerState.ControllerBtnState |= BtnFlags::BtnLeft;
		}
		if (state.rgbButtons[1])
		{
			m_controllerState.ControllerBtnState |= BtnFlags::BtnDown;
		}
		if (state.rgbButtons[2])
		{
			m_controllerState.ControllerBtnState |= BtnFlags::BtnRight;
		}
		if (state.rgbButtons[3])
		{
			m_controllerState.ControllerBtnState |= BtnFlags::BtnUp;
		}
		...
		{
			m_controllerState.ControllerBtnState |= BtnFlags::BtnShare;
		}
		if (state.rgbButtons[9])
		{
			m_controllerState.ControllerBtnState |= BtnFlags::BtnOptions;
		}

		if (state.rgbButtons[10])
		{
			m_controllerState.ControllerBtnState |= BtnFlags::BtnL3;
		}
		if (state.rgbButtons[11])
		{
			m_controllerState.ControllerBtnState |= BtnFlags::BtnR3;
		}
